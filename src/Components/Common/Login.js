...
const Login = (props) => {
  ...
  const submitLoginAPI = async (nationalNumber) => {
    try {
      ...
      if (response.data.success) {
        setCurrentUser({ mobile: nationalNumber });
  ...
  const handleSubmit = async (e) => {
    ...
    if (step === 1) {
      ...
      try {
        ...
        // FIX: `currentUser` is still null at this point (it's only set
        // inside submitLoginAPI, further down). Use the `phone` state,
        // which is what actually holds the number the user just typed.
        const nationalNumber = phone;

        const sessionResponse = await authAxios().post("/auth/session", {
          mobile: nationalNumber,
        });
    ...
    // ✅ OTP Verification
      ...
      try {
        ...
        if (response.data.success) {
          const { token, staff, session_expires_at } = response.data.data;
          ...
          dispatch(setSessionExpiresAt(session_expires_at));
          // dispatch(setSessionExpiresAt(new Date(Date.now() + 10000).toISOString()));
  ...
  const handleResendOtp = async () => {
    try {
      ...
      if (response.data.success) {
        setCurrentUser({ mobile: currentUser.mobile });
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {step === 1 && (
              ...
              >
                ...
                <span className="text-black text-md mr-2 select-none">
                  +91
                </span>
                <input
                  type="tel"
            ...
            <div className="flex justify-center gap-4">
              <button
                onClick={async () => {
                  ...
                  if (pendingLogin) {
                    // FIX: same issue as above — currentUser isn't set yet
                    // at this point, so fall back to the `phone` state.
                    const nationalNumber = phone;
                    await submitLoginAPI(nationalNumber);
