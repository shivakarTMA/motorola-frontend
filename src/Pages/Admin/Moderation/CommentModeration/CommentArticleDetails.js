...
const CommentArticleDetails = ({
  ...
  const getHtmlContent = () => {
    if (!postDetails?.comment?.post?.content) return "";

    try {
      const ops = JSON.parse(postDetails?.comment?.post?.content);
      ...
      console.error(err);
      return postDetails?.comment?.post?.content;
    }
  };

  const html = getHtmlContent();
  const media = postDetails?.comment?.post?.media || [];
  ...
  const infoRows = [
    ...
    ["Tribe Group", showValue(postDetails?.comment?.post?.circle?.circleGroup?.name)],
    ["Tribe Name", showValue(postDetails?.comment?.post?.circle?.name)],
                    ...
                    <div className="border-b md:border-b-0 md:border-r border-gray-100">
                      ...
                      <div className="px-5 py-3">
                        <div className="max-h-[40vh] overflow-auto">
                          {postDetails?.comment?.post?.title && (
                        ...
                        <div className="mt-3 flex items-center gap-5 text-black px-2">
                          <span className="flex items-center gap-2 text-sm">
                            <FiHeart size={20} />
                            {showValue(postDetails?.comment?.post?.likes_count)}
                          </span>
                          <span className="flex items-center gap-2 text-sm">
                            <FiMessageSquare size={20} />
                            {showValue(postDetails?.comment?.post?.comments_count)}
                          </span>
                        </div>
                      </div>
                    </div>
...
export default CommentArticleDetails;
