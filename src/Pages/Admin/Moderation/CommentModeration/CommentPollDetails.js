...
const CommentPollDetails = ({
  ...
  const getHelpTextHtml = () => {
    if (!postDetails?.comment?.poll?.help_text) return "";

    try {
      const ops = JSON.parse(postDetails?.comment?.poll?.help_text);
      const converter = new QuillDeltaToHtmlConverter(ops, {});
      return converter.convert();
    } catch (err) {
      console.error(err);
      return postDetails?.comment?.poll?.help_text;
    }
  };

  const helpTextHtml = getHelpTextHtml();
  const options = postDetails?.comment?.poll?.options || [];
  const totalVotes = postDetails?.comment?.poll?.total_votes || 0;
  ...
  const infoRows = [
    ...
    ["Ending at", formatWithTimeDate(postDetails?.comment?.poll?.poll_ends_at)],
    ["Content ID", showValue(postDetails?.content_id)],
    ["Tribe Group", showValue(postDetails?.comment?.poll?.circle?.circleGroup?.name)],
    ["Tribe Name", showValue(postDetails?.comment?.poll?.circle?.name)],
    ["Flagged Keywords", showValue(postDetails?.matchedKeyword?.keyword)],
    ["Action Take", showValue(postDetails?.action)],
    ...(postDetails?.moderationActions?.[0]?.reason
        ? [["Reason", showValue(postDetails.moderationActions[0].reason)]]
        : []),
  ];
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div className="border-b md:border-b-0 md:border-r border-gray-100">
                      ...
                      <div className="px-5 py-3">
                        {postDetails?.comment?.poll?.question && (
                          <h3 className="mb-2 lg:text-[24px] text-lg font-semibold text-gray-900">
                            {showValue(postDetails?.comment?.poll?.question)}
                          </h3>
                        )}
                        {helpTextHtml ? (
                        ...
                        <div className="flex gap-2 text-sm pt-3">
                            <span className="text-black font-semibold">Flagged Comment:</span>
                            <span>{postDetails?.comment?.content || "--"}</span>
                        </div>
                        ...
                        <div className="mt-4 flex items-center justify-between text-black px-2">
                          <div className="flex items-center gap-5">
                            <span className="flex items-center gap-2 text-sm">
                              <FiHeart size={20} />
                              {showValue(postDetails?.comment?.poll?.likes_count)}
                            </span>
                            <span className="flex items-center gap-2 text-sm">
                              <FiMessageSquare size={20} />
                              {showValue(postDetails?.comment?.poll?.comments_count)}
                            </span>
                          </div>
...
export default CommentPollDetails;
