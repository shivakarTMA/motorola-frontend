...
const ViewArticleDetails = ({ open, onClose, editId, onSuccess }) => {
    ...
    const fetchPostDetails = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setPostDetails(data);
          setEnabled(Boolean(data.is_featured));
        }
      } catch (err) {
        toast.error(
  ...
  const handleConfirm = async () => {
    try {
      ...
      const payload = {
        is_featured: pendingValue,
      };

      console.log(payload);

      const res = await authAxios().put(`/post/${editId}`, payload);
  ...
  const handleCancel = () => {
    setPendingValue(enabled); // reset pending value
    setConfirmOpen(false);
  };
